...
onMounted(() => {
  ...
  sock.on('acceptCall', async () => {
    if (caller.value) { // 发送方
      ...
      peer.value.onaddstream = (event: any) => { 
        console.log('用户A收到用户B远端音视频流',event.stream)
  ...
  sock.on('sendOffer', async (offer: any) => {
    if (called.value) {
      ...
      // 获取对方的音视频流
      peer.value.onaddstream = (event: any) => {  
        console.log('用户B收到用户A远端音视频流',event.stream)
  ...
  sock.on('receiveAnswer', (answer: any) => {
    if (caller.value) {
      console.log('发送方收到接收方的answer', answer)
      // 设置远端描述信息
  ...
  <div class="flex items-center flex-col text-center p-12 h-screen">
    <div class="relative h-full mb-4">
      ...
      <div v-if="caller && calling" class="absolute top-2/3 left-36 flex flex-col items-center">
        ...
        <img @click="hangUp" src="../public/refuse.svg" class="w-16 cursor-pointer" alt="">
      </div>
      <div v-if="called && calling" class="absolute top-2/3 left-32 flex flex-col items-center">
        <p class="mb-4 text-white">收到视频邀请...</p>
        <div class="flex">
          <img @click="hangUp" src="../public/refuse.svg" class="w-16 cursor-pointer mr-4" alt="">
          <img @click="acceptCall" src="../public/accept.svg" class="w-16 cursor-pointer" alt="">
        </div>
      </div>
    </div>
